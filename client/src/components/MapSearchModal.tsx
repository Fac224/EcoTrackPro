...
export function MapSearchModal({ isOpen, onClose, searchParams }: MapSearchModalProps) {
  ...
  if (searchParams.startTime && searchParams.startTime !== "any_time") queryParams.startTime = searchParams.startTime;
  if (searchParams.endTime && searchParams.endTime !== "any_time") queryParams.endTime = searchParams.endTime;
      ...
      <DialogContent className="sm:max-w-[90vw] max-h-[90vh] overflow-hidden flex flex-col p-0">
        <DialogHeader className="p-4 border-b">
          ...
          <div className="flex flex-wrap text-sm text-gray-500 mt-1">
            ...
            {searchParams.startTime && searchParams.startTime !== "any_time" && 
              <span className="mr-3">From: {formatTimeString(searchParams.startTime)}</span>}
            {searchParams.endTime && searchParams.endTime !== "any_time" && 
              <span>To: {formatTimeString(searchParams.endTime)}</span>}
          </div>
